...
import type { Budget, BudgetSummary } from "../../services/budgetService";
import {
  BarChart,
  Bar,
  XAxis,
...
function Budget() {
  ...
  const categories = ["Maintenance", "Procurement", "Utilities", "Events", "Projects"];

  const COLORS = ["#1976d2", "#ff9800", "#4caf50", "#9c27b0", "#f44336"];
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        <Grid item xs={12} sm={6} md={4}>
          <Card sx={{ bgcolor: "#e3f2fd" }}>
            <CardContent>
              <Typography variant="body2" sx={{ color: "text.secondary" }}>Planned</Typography>
                ...
                <Box key={cat.category} sx={{ mb: 2 }}>
                  <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <Typography variant="body2">
                      {cat.spent > 0 ? ((cat.spent / cat.planned) * 100).toFixed(0) : 0}%
                    </Typography>
                  </Box>
                  <LinearProgress
                    variant="determinate"
                    value={cat.planned > 0 ? (cat.spent / cat.planned) * 100 : 0}
                    sx={{ height: 8, borderRadius: 4 }}
                    color={cat.spent > cat.planned ? "error" : "primary"}
                  />
                </Box>
              ))}
            </Box>
        ...
        <TableContainer>
          <Table>
            ...
            <TableBody>
              {budgets.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} align="center">No budget entries found</TableCell>
                </TableRow>
              ) : (
                budgets.map((budget) => (
                  <TableRow key={budget.id}>
                    <TableCell>{budget.category}</TableCell>
                    <TableCell align="right">R$ {budget.planned.toFixed(2)}</TableCell>
                    <TableCell align="right">R$ {budget.spent.toFixed(2)}</TableCell>
                    <TableCell align="right" sx={{ color: (budget.planned - budget.spent) < 0 ? "error.main" : "success.main" }}>
                      R$ {(budget.planned - budget.spent).toFixed(2)}
                    </TableCell>
                    <TableCell>{new Date(2000, budget.month - 1).toLocaleString('default', { month: 'long' })}</TableCell>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleSaveBudget}>Create</Button>
        </DialogActions>
      </Dialog>
